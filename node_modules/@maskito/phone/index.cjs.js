...
function cutInitCountryCodePreprocessor({ countryIsoCode, metadata, }) {
    ...
    return ({ elementState, data }) => {
        ...
        try {
            const phone = core.parsePhoneNumber(value, countryIsoCode, metadata);
...
function validatePhonePreprocessorGenerator({ prefix, countryIsoCode, metadata, }) {
    return ({ elementState, data }) => {
        ...
        if (value && !value.startsWith(cleanCode) && !data) {
            const formatter = new core.AsYouType({ defaultCountry: countryIsoCode }, metadata);
        ...
        try {
            const validationError = core.validatePhoneNumberLength(data, { defaultCountry: countryIsoCode }, metadata);
            if (!validationError) {
...
function maskitoPhoneNonStrictOptionsGenerator({ defaultIsoCode, metadata, separator = '-', }) {
    ...
    return Object.assign(Object.assign({}, core$1.MASKITO_DEFAULT_OPTIONS), { mask: ({ value }) => {
